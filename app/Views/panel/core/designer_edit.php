<div class="col">
    <div class="card card-primary col-12  p0">
        ...
        <!-- /.card-header -->
        <!-- form start -->
        <form role="form" class="form" id="form" action="<?= base_url('panel/designer/update') ?>" method="POST">
            <?= csrf_field() ?>
            ...
            <div class="card-body">
                <div class="row">
                    <div class="form-group col-12 col-sm-3">
                        ...
                        <input type="text" class="form-control" id="d_name" name="name" placeholder="Lütfen Dizayn İsmini Yazınız" required value="<?= $designer->designer_name; ?>">
                        <input type="hidden" name="designer_id" value="<?=$designer->designer_id?>" id="data">
                    </div>
                    <div class="form-group col-12 col-sm-3">
                        <label for="v_name">View Name</label>
                        <input type="text" class="form-control" id="v_name" name="view_name" placeholder="View Name" required value="<?= $designer->view_name; ?>">
                    </div>

                    <div class="form-group col-12 col-sm-3">
                        <label for="Sort">Dizayn Sıra</label>
                        <input type="text" class="form-control" id="Sort" name="sortDesign" placeholder="Sort" value="<?= $designer->designer_sort_order; ?>">
                    </div>
                ...
                <div class="row" id="groups">
                    ...
                    <?php
                    // Designer Data Veriables and Sort Order Here --------------
                    $designer_data = json_decode($designer->designer_data);
                    usort($designer_data, function ($a, $b) {
                        if ($a->groupOrder == '')
                            $a->groupOrder = 99;
                        if ($b->groupOrder == '')
                            $b->groupOrder  = 98;
                    ...
                    foreach ($designer_data as $row) {
                        //echo $row->groupId;
                        //Create Groups ------------------------
                    ?>
                        <div class="card card-danger col-12 p-0 groups" id="<?= $row->groupId ?>">
                            <div class="card-header col">
                                <div class="row">
                                    <div class="form-group col-12 col-sm-3">
                                        <label for="<?= $row->groupId ?>_groups_name">Group Name</label>
                                        <?php foreach ($languages as $lang) {
                                            ...
                                            foreach ($langData as $data) {
                                                $languageData = json_decode($data->language_data);
                                        ...
                                        ?>
                                            <input type="hidden" name="group_id_<?= $lang->language_code ?>" value="<?=$groupId;?>">
                                            <input class="form-control" value="<?= $groupName ?>" id="<?= $row->groupId ?>_groups_name_<?= $lang->language_code ?>" name="<?= $row->groupId ?>_groups_name_<?= $lang->language_code ?>" placeholder="Group Name <?= $lang->language_code ?>">
                                        <?php } ?>

                                    </div>
                                    <div class="form-group col-12 col-sm-3">
                                        <label for="<?= $row->groupId ?>_groups_row">Order</label>
                                        <input class="form-control" value="<?= $row->groupOrder ?>" id="<?= $row->groupId ?>_groups_row" name="<?= $row->groupId ?>_groups_row" placeholder="Order">
                                    </div>
                                    <div class="form-group col-12 col-sm-3">
                                        <label for="<?= $row->groupId ?>_groups_type">Group Type</label>
                                        <select class="form-control" name="<?= $row->groupId ?>_groups_type" id="<?= $row->groupId ?>_groups_type">
                                ...
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                    <span aria-hidden="true" onclick="deleteGroup('<?= $row->groupId ?>')">Delete Group ×</span>
                                </button>
                            </div>
                            <div class="card-body">
                                <div class="row">
                                    <div class="form-group col-12 col-sm-3">
                                        <label for="<?= $row->groupId ?>_form_type">Form Type</label>
                                        <select class="form-control" name="<?= $row->groupId ?>_form_type" id="<?= $row->groupId ?>_form_type">
                                    ...
                                    <div class="col-12 col-sm-3 align-self-end bot-15">
                                        <div class="card-tools col">
                                            <button type="button" class="btn btn-outline-info btn-sm" onclick="addForm('<?= $row->groupId ?>')">
                                ...
                                <hr>
                                <div class="row" id="<?= $row->groupId ?>_forms">
                                    <?php
                                    //--------------------------------------
                                    // Group Add Ended  --------------------
                                    //--------------------------------------

                                    usort($row->forms, function ($a, $b) {
                                        if ($a->formElementRow == "")
                                            $a->formElementRow = 99;
                                    ...
                                    ?>
                                        <div class="card card-outline card-info col-12 col-sm-3 forms" id="<?= $formElementId ?>">
                                            <div class="card-header">
                                                <h3 class="card-title" id="<?= $formElementId ?>_title"><?= $form->formElementType ?></h3>
                                                <input type="hidden" class="data" name="<?= $formElementId ?>_elem_type" value="<?= $form->formElementType ?>">
                                                <div class="card-tools">
                                                    <button type="button" class="btn btn-tool" data-card-widget="collapse">
                                                        <i class="fas fa-minus"></i>
                                                    </button>
                                                    <button type="button" class="close">
                                                        <span aria-hidden="true" onclick="deleteForm('<?= $formElementId ?>')">×</span>
                                                    </button>
                                                </div>
                                            </div>
                                            <div class="card-body related_forms" style="display: block;">
                                                <div class="form-group col-12">
                                                    <label for="<?= $formElementId ?>_elem_label">Label</label>
                                                    <?php
                                                    foreach ($languages as $lang) {
                                                        $lang_id = $lang->language_id;
                                                        $lang_data_id =$lang_id;
                                                        $formElementName = '';
                                                        foreach ($langData as $data) {
                                                            $languageData = json_decode($data->language_data);
                                                            if ($data->language_id == $lang_id) {
                                                                $lang_data_id = $data->designer_data_languages_id;
                                                                foreach ($languageData as $item) {
                                                                    if ($item->groupId == $row->groupId) {
                                                                        foreach ($item->forms as $elem) {
                                                                            if($elem->formElementId == $formElementId)
                                                                            {
                                                                                $formElementName = $elem->formElementLabel;
                                                                                break 2; // İç içe döngülerden çık
                                                                            }
                                                                        }
                                                                    }
                                                    ...
                                                    ?>
                                                        <input type="hidden" name="designer_data_languages_id_<?= $lang->language_code ?>" value="<?= $lang_data_id;?>">
                                                        <input class="form-control data" value="<?= $formElementName ?>" id="<?= $formElementId ?>_elem_label_<?= $lang->language_code ?>" name="<?= $formElementId ?>_elem_label_<?= $lang->language_code ?>" placeholder="Label Name">
                                                    <?php }
                                                    ?>
                                                ...
                                                <div class="form-group col-12">
                                                    <label for="<?= $formElementId ?>_elem_name">Name</label>
                                                    <input class="form-control data" value="<?= $form->formElementName ?>" id="<?= $formElementId ?>_elem_name" name="<?= $formElementId ?>_elem_name" placeholder="Database Name">
                                                </div>
                                                <div class="form-group col-12">
                                                    <label for="<?= $formElementId ?>_elem_row">Row</label>
                                                    <input type="number" class="form-control data" value="<?= $form->formElementRow ?>" id="<?= $formElementId ?>_elem_row" name="<?= $formElementId ?>_elem_row" placeholder="Row">
                                                </div>
                                                <div class="form-group col-12">
                                                    <label for="<?= $formElementId ?>_elem_default">Default Value</label>
                                                    <input class="form-control data" value="<?= $form->formElementDefault ?>" id="<?= $formElementId ?>_elem_default" name="<?= $formElementId ?>_elem_default" placeholder="Default Value">
                                                </div>

                                                <div class="form-group col-12">
                                                    <label for="<?= $formElementId ?>_elem_required">Required?</label>
                                                    <select class="form-control data" name="<?= $formElementId ?>_elem_required" id="<?= $formElementId ?>_elem_required">
                                                        <option value="0" <?php if ($form->formElementRequired == 0) echo 'selected'; ?>>No</option>
                                                        <option value="1" <?php if ($form->formElementRequired == 1) echo 'selected'; ?>>Yes</option>
                                                    </select>
                                                </div>
                                                <!-- Form content -->
                                            </div>
